 ...
 */
export const json2env = (options: ConvertOptions): void => {
  const { inputFilename, outputFilename } = options;
  try {
    const jsonData = fs.readFileSync(inputFilename ?? '../env.json', 'utf8');
    ...
    fs.writeFileSync(outputFilename ?? '../.env', envContent);
